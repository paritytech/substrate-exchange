//! Utility for interacting with a substrate blockchain.

use futures::{
    future::{self, Future, IntoFuture},
    stream::Stream,
};
use jsonrpc_core_client::{transports::ws, RpcChannel, RpcError, TypedSubscriptionStream};
use log;
use node_runtime::{Call, Runtime, UncheckedExtrinsic};
...
pub enum Error {
    ...
    Io(std::io::Error),
    /// Json RPC error.
    Rpc(jsonrpc_core_client::RpcError),
    /// Stream terminated
    StreamTerminated,
    /// Extrinsic usurped
    ExtrinsicUsurped,
...
impl From<RpcError> for Error {
    fn from(error: jsonrpc_core_client::RpcError) -> Self {
        Error::Rpc(error)
    }
}
...
impl Rpc {
                ...
                stream
                    .filter_map(|status| {
                        match status {
                            ...
                            Status::Usurped(_) => Some(Err(Error::ExtrinsicUsurped)),
                            Status::Dropped => Some(Err(Error::ExtrinsicDropped)),
                            Status::Invalid => Some(Err(Error::ExtrinsicInvalid)),
                        }
                    })
                    .into_future()
                    .map_err(|(e, _)| e.into())
    ...
    fn create_and_submit_extrinsic(
        ...
        let genesis_hash = self
            .fetch_genesis_hash()
            .map_err(Into::into)
            .and_then(|genesis_hash| future::result(genesis_hash.ok_or(Error::GenesisNotFound)));
...
fn wait_for_block_events(
    ...
    let ext_index = signed_block
        ...
        .ok_or(Error::ExtrinsicNotFound(ext_hash))
        .into_future();
